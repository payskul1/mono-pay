...
const checkIfUpdateNeeded = (existingData, newData) => {
    ...
    if (!existingData.first_name || existingData.first_name === 'undefined' || existingData.first_name.trim() === '') {
        if (newData.firstName && newData.firstName.trim() !== '') {
            updatePayload.first_name = newData.firstName;
    ...
    if (!existingData.last_name || existingData.last_name === 'undefined' || existingData.last_name.trim() === '') {
        if (newData.lastName && newData.lastName.trim() !== '') {
            updatePayload.last_name = newData.lastName;
...
//     console.log(id);
//     const customerDetails = await fetch(`${API_BASE_URL}/v2/customers/${id}`,{
//         method: 'GET',
//         headers: {
//             'Content-Type': 'application/json',
//             'accept': 'application/json',
...
//     });
//     const result = await customerDetails.json();
//     console.log();
//     // const response = await fetch(`${API_BASE_URL}/v2/payments/initiate`, {
//     //     method: 'POST',
//     //     headers: {
...
//     // const result = await response.json();
//     // console.log(mandatePayload.customer.id);
//     // console.log("from initiate: EMAIL "+ mandatePayload.email," PHONE NO ", mandatePayload.phone, "ADDRESS ", mandatePayload.address);
...
export const initiateMandate = async (mandatePayload) => {
    ...
    //     firstName: customerResult.data.first_name || '',
    //     lastName: customerResult.data.last_name === 'undefined' ? '' : customerResult.data.last_name || '',
    //     email: customerResult.data.email || '',
    //     address: customerResult.data.address || 'N/A',
    //     phone: '', // Not provided in the API response, you may need to get this from somewhere else
    //     identity: {
    //         type: customerResult.data.identification_type || 'bvn',
    //         number: customerResult.data.identification_no || customerResult.data.bvn || '',
    //     }
    // };
    
    // console.log('Transformed customer data:', transformedCustomerData);
    
    // Update the mandate payload with the transformed customer data
    // const updatedMandatePayload = {
    //     ...mandatePayload,
    //     customer: {
    //         ...mandatePayload.customer,
