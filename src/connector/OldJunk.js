    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div>
        ...
        >
          ...
          <option className="text-black" value="0.25">
            3 Months
          </option>
          <option className="text-black" value="0.5">
            6 Months
          </option>
          <option className="text-black" value="0.75">
            9 Months
          </option>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-blue-600/20 border border-blue-400/30 rounded-lg p-6">
        ...
        <p className="text-white/80 mb-4">
          Connect your bank account securely to enable automatic loan
          repayments.
        </p>
  ...
  <div className="text-center space-y-8">
    ...
    <div>
      ...
      <p className="text-purple-300 text-sm">
        You will receive an email confirmation shortly with your application
        details.
      </p>
    </div>
  ...
  <div className="flex justify-between mb-8 relative">
    {steps.map((step, index) => (
      <div key={step.id} className="flex flex-col items-center flex-1 relative">
        ...
        <span
          className={`text-xs text-center hidden lg:flex ${
...
const NavigationButtons = ({
  ...
  <div className="grid grid-cols-2 justify-between mt-8 lg:flex">
    <button
      ...
      className={`px-4 py-3 mr-2 rounded-xl font-medium transition-all duration-300 flex items-center ${
        currentStep === 1
...
const StudentLoan = () => {
  ...
  const calculateMonthlyPayment = () => {
    const loanAmount = parseFloat(formData.loanAmount) || 0;
    const repaymentPeriodInMonths =
      (parseFloat(formData.repaymentTerm) || 0) * 12;
    if (loanAmount > 0 && repaymentPeriodInMonths > 0) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isStepValid(currentStep)) {
      alert(
        "Please complete all required information and provide consent for automatic debit."
      );
      return;
    }
  ...
  const renderCurrentStep = () => {
    switch (currentStep) {
      ...
      case 6:
        return (
          <SuccessPage applicationId={applicationId} formData={formData} />
        );
      default:
        return (
          <PersonalInformation
